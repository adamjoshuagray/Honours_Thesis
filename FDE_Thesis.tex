...
\end{align}
along with the initial and ``final'' conditions
\begin{align}
	\label{eq:fde-singular-ic-1}
	y(0) = 0 \\
	\label{eq:fde-singular-ic-2}
	y'(0) = 0 \\
	\label{eq:fde-singular-ic-3}
	\lim_{t\lra \infty} y(t) = 0 \\
	\label{eq:fde-singular-ic-4}
	\lim_{t\lra \infty} y'(t) = 0.
\end{align}
...
\begin{lemma}[Mellin Transform of Integer Order Derivatives]
	...
	\begin{align*}
		\mellin{f^{(n)}(t)} = \sum_{k=0}^\infty \frac{\Gamma(1-r+k)}{\Gamma(1-s)} \left[ f^{(n-k-1)}(t)t^{r-k-1}\right]_{s=0}^{s\lra\infty} + \frac{\Gamma(1-r-n)}{\Gamma(1-r)}F(r-n).
	\end{align*}
\end{lemma}
